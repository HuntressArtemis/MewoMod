...
{ 
	public class MewOreTile : ModTile
	{
        ...
        public override void SetStaticDefaults() {
			...
			AddMapEntry(new Color(188, 0, 204), name);

			DustType = 84;
			HitSound = SoundID.Item57;

            MineResist = 4f;
            MinPick = 200;   
        }
	}
        // MewOreSystem contains code related to spawning MewOre. It contains both spawning ore during world generation, seen in ModifyWorldGenTasks, and spawning ore after defeating a boss, seen in BlessWorldWithMewOre and MinionBossBody.OnKill.
	public class MewOreSystem : ModSystem
	{
		public static LocalizedText MewOrePassMessage { get; private set; }
		public static LocalizedText BlessedWithMewOreMessage { get; private set; }

		public override void SetStaticDefaults() {
			MewOrePassMessage = Mod.GetLocalization($"WorldGen.{nameof(MewOrePassMessage)}");
			BlessedWithMewOreMessage = Mod.GetLocalization($"WorldGen.{nameof(BlessedWithMewOreMessage)}");
		}
		...
		public void BlessWorldWithMewOre() {
			...
			ThreadPool.QueueUserWorkItem(_ => {
				...
				if (Main.netMode == NetmodeID.SinglePlayer) {
					Main.NewText(BlessedWithMewOreMessage.Value, 50, 255, 130);
				}
				else if (Main.netMode == NetmodeID.Server) {
					ChatHelper.BroadcastChatMessage(BlessedWithMewOreMessage.ToNetworkText(), new Color(50, 255, 130));
				}
				...
				for (int iteration = 0; iteration < splotches; iteration++) {
					...
					// OreRunner will spawn MewOre in splotches. OnKill only runs on the server or single player, so it is safe to run world generation code.
					WorldGen.OreRunner(i, j, WorldGen.genRand.Next(5, 9), WorldGen.genRand.Next(5, 9), (ushort)ModContent.TileType<MewOreTile>());
				}
			});
		}
		...
		public override void ModifyWorldGenTasks(List<GenPass> tasks, ref double totalWeight) {
			...
			if (ShiniesIndex != -1) {
				...
				// MewOrePass is a class seen bellow
				tasks.Insert(ShiniesIndex + 1, new MewOrePass("Mewo Ores", 237.4298f));
			}
		}
	}

	public class MewOrePass : GenPass
	{
		public MewOrePass(string name, float loadWeight) : base(name, loadWeight) { 			
		}
		...
		protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration) {
			...
			progress.Message = MewOreSystem.MewOrePassMessage.Value;
			...
			for (int k = 0; k < (int)(Main.maxTilesX * Main.maxTilesY * 6E-05); k++) {
				...
				WorldGen.TileRunner(x, y, WorldGen.genRand.Next(3, 6), WorldGen.genRand.Next(2, 6), ModContent.TileType<MewOreTile>());
